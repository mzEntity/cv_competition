...
def dualTest(device, test_loader, model, criterion):
    """
    Evaluate the model on the test set.
    ...
    with torch.no_grad():  # No need to track gradients during testing
        with open('output.txt', 'w', encoding='utf-8') as file:
            for (names, is_face, images), labels in tqdm(test_loader, desc="Eval Processing:"):
                images, labels = images.to(device), labels.to(device).float()

                # Forward pass
                outputs = model(is_face, images).squeeze(1)  # Ensure outputs are 1D
...
if __name__ == "__main__":
    ...
    body_model_path = "backup//body.pth.tar"
    face_model_path = "backup//face.pth.tar"
    model = DualModel(os.path.join(root_dir, body_model_path), os.path.join(root_dir, face_model_path))
    test_dataset = get_dualDataset(root_dir)
